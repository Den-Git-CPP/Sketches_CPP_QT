...
{
    std::stringstream ss (forecast_str);
    std::string item;  // токен слово
    char delim{ ' ' }; // разделитель
    // пока поток есть извлекаем item
    while (std::getline (ss, item, delim)) {
        // если поток закончился   ввыодим сообщение

        if ((item == "METAR") || (item == "TAF")) {
            (item == "METAR") ? Metar_flag = true : Taf_flag = true;
        }

        if (Metar_flag) {
            //            set_convert_Forcast (metar, item); //
        }
        else if (Taf_flag) {
            //          set_convert_Forcast (taf, item); //
        }
        else {
            std::cout << "Dont ForecastObj\n";
        }
        if (ss.eof ()) {
            std::cout << "END ForecastObj\n";
            break;
        }
    }
}
template <class T>
void Storage_Forecast::set_convert_Forcast (std::unique_ptr<T>& s, const std::string& input_word)
{
