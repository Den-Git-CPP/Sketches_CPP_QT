    ...
    if (reader.readNextStartElement ()) {
        if (reader.name () == QString ("response")) {
            while (reader.readNextStartElement ()) {
                ...
                if (reader.name () == QString ("data_source") && reader.attributes ().hasAttribute ("name")) {
                    QString type_str = reader.attributes ().value ("name").toString (); // read atributes
                    qDebug () << reader.name () << "\t" << type_str;
                    reader.skipCurrentElement ();
                }
                if (reader.name () == QString ("request") && reader.attributes ().hasAttribute ("type")) {
                ...
                if (reader.name () == QString ("data") && reader.attributes ().hasAttribute ("num_results")) {
                    QString type_str = reader.attributes ().value ("num_results").toString (); // read atributes
                    qDebug () << reader.name () << "\t" << type_str;
                }
                    ...
                    while (reader.readNextStartElement ()) {
                        ...
                        if (reader.name () == QString ("forecast")) {
                            while (reader.readNextStartElement ()) {
                                ...
                                if (reader.name () == QString ("time_becoming")) {
                                    qDebug () << reader.name () << "\t" << reader.readElementText ();
                                }
                                if (reader.name () == QString ("wind_dir_degrees")) {
                                    qDebug () << reader.name () << "\t" << reader.readElementText ();
                                }
                                if (reader.name () == QString ("wind_speed_kt")) {
                                    qDebug () << reader.name () << "\t" << reader.readElementText ();
                                }
                                if (reader.name () == QString ("wind_gust_kt")) {
                                    qDebug () << reader.name () << "\t" << reader.readElementText ();
                                }
                                if (reader.name () == QString ("visibility_statute_mi")) {
                                    qDebug () << reader.name () << "\t" << reader.readElementText ();
                                }
                                ...
                                if (reader.name () == QString ("sky_condition") && reader.attributes ().hasAttribute ("sky_cover") &&
                                    reader.attributes ().hasAttribute ("cloud_base_ft_agl")) {
                                    QString sky_cover_str = reader.attributes ().value ("sky_cover").toString ();
                                    QString cloud_base_ft_agl_str =
                                      reader.attributes ().value ("cloud_base_ft_agl").toString (); // read atributes
