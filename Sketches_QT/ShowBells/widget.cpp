...
Widget::Widget (QWidget* parent) : QWidget (parent)
{
    ...
    label->setText ("Компьютер в работе");

    vbox = new QVBoxLayout ();
    vbox->addWidget (label);

    this->setLayout (vbox);
    bells = new lb_bells (this->parentWidget ());

    timer_show_bells = new QTimer (this);
    timer_show_bells->setInterval (10000); // интервал 10 сек
...
{
    bells->move (this->mapToGlobal (QPoint (-50, 0)));
    bells->show ();
